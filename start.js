function loseGame() {
  alert("Thousten has Lost; Thousten had brought dishonor to us all; Thoust is at the level of a Manabadi Teacher");
  gameState="over";
}
function winGame() {
  alert("THOUSTEN HAS ACCLAIMED VICTORY");
  endLevel();
}
...
function leveltwo() {
  hero.health+=Math.floor((hero.maxHealth-hero.health)/2); increaseIntensity(1); waveSpeed-=25; gameLength+=100; spawners.forEach((spawner,i)=>{spawner.maxPower=15;})
}
function levelthree() {
  hero.health+=Math.floor((hero.maxHealth-hero.health)/2); increaseIntensity(2); waveSpeed-=25; gameLength+=200; spawners.forEach((spawner,i)=>{spawner.maxPower=16;})
}
function levelfour() {
  hero.health+=Math.floor((hero.maxHealth-hero.health)/2); increaseIntensity(2); waveSpeed-=50; gameLength+=250; spawners.forEach((spawner,i)=>{spawner.maxPower=18;})
}
function levelfive() {
  hero.health+=Math.floor((hero.maxHealth-hero.health)/2); increaseIntensity(3); gameLength+=500; spawners.forEach((spawner,i)=>{spawner.maxPower=19;})
}
function levelsixplus() {hero.health+=Math.floor((hero.maxHealth-hero.health)/2); increaseIntensity(1); gameLength+=100; spawners.forEach((spawner,i)=>{spawner.maxPower=level+15;})}
