function enemySpawner(x, y, rad, rate) {
  ...
  this.enemypowerlevels = [[this.newZombie, 2],[this.newZombie, 2], [this.newImp,6], [this.newImp,6], [this.newImp,6], [this.newCharger,9], [this.newTotem, 11], [this.newTotem, 11], [this.newHealer, 9], [this.newHealer, 9]];
}
const enemyLibrary = {
  ...
  Imp: function(x,y,dx,dy,strength) {
    this.x=x;this.y=y;this.dx=dx;this.dy=dy;this.rad=3.6-strength/6; this.health=10+strength*6; this.weight=2.5; this.type="enemy";
    this.update = function(index) {this.move();this.draw(); if(this.health<=0){this.remove(index)}}
  ...
  Charger: function(x,y,dx,dy,strength) {
    ...
    this.remove = function(index) {didDelete=true; enemies.splice(index,1); herospeed*=1.2;}
